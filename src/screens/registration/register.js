import React, { useState } from 'react'
import TextField from '@mui/material/TextField';
import OutlinedInput from '@mui/material/OutlinedInput';
import InputAdornment from '@mui/material/InputAdornment';
import Checkbox from '@mui/material/Checkbox';
import FormControlLabel from '@mui/material/FormControlLabel';
import { Link } from 'react-router-dom';
import Button from '@mui/material/Button';

export const Registration = () => {
  const [show, setShow] = useState("password")
  const toggle = () =>{
    if (show=="password"){
      setShow("text")
    }
    else{
      setShow("password")
    }
  }
  return (
    <div className='w-screen h-screen flex flex-col justify-center items-center gap-5'>
     
      <div className='flex flex-row w-[710px] h-[600px] border-[1px] rounded-md px-10 py-12'>
      
        <div className='flex flex-col w-[55%] h-[100%] gap-3'>
       
        <svg viewBox="0 0 75 24" width="75" height="24" xmlns="http://www.w3.org/2000/svg" 
                aria-hidden="true" class="l5Lhkf"><g id="qaEJec"><path fill="#ea4335" 
                d="M67.954 16.303c-1.33 0-2.278-.608-2.886-1.804l7.967-3.3-.27-.68c-.495-1.33-2.008-3.79-5.102-3.79-3.068
                0-5.622 2.41-5.622 5.96 0 3.34 2.53 5.96 5.92 5.96 2.73 0 4.31-1.67 4.97-2.64l-2.03-1.35c-.673.98-1.6 
                1.64-2.93 1.64zm-.203-7.27c1.04 0 1.92.52 2.21 1.264l-5.32 2.21c-.06-2.3 1.79-3.474 3.12-3.474z"></path>
                </g><g id="YGlOvc"><path fill="#34a853" d="M58.193.67h2.564v17.44h-2.564z"></path></g><g id="BWfIk">
                    <path fill="#4285f4" d="M54.152 8.066h-.088c-.588-.697-1.716-1.33-3.136-1.33-2.98 0-5.71 2.614-5.71 
                    5.98 0 3.338 2.73 5.933 5.71 5.933 1.42 0 2.548-.64 3.136-1.36h.088v.86c0 2.28-1.217 3.5-3.183 3.5-1.61
                    0-2.6-1.15-3-2.12l-2.28.94c.65 1.58 2.39 3.52 5.28 3.52 3.06 0 5.66-1.807 5.66-6.206V7.21h-2.48v.858zm-3.006
                    8.237c-1.804 0-3.318-1.513-3.318-3.588 0-2.1 1.514-3.635 3.318-3.635 1.784 0 3.183 1.534 3.183 3.635 0 2.075-1.4
                    3.588-3.19 3.588z"></path></g><g id="e6m3fd"><path fill="#fbbc05" d="M38.17 6.735c-3.28 0-5.953 2.506-5.953 5.96
                    0 3.432 2.673 5.96 5.954 5.96 3.29 0 5.96-2.528 5.96-5.96 0-3.46-2.67-5.96-5.95-5.96zm0 9.568c-1.798 0-3.348-1.487-3.348-3.61
                    0-2.14 1.55-3.608 3.35-3.608s3.348 1.467 3.348 3.61c0 2.116-1.55 3.608-3.35 3.608z"></path></g><g id="vbkDmc"><path fill="#ea4335" 
                    d="M25.17 6.71c-3.28 0-5.954 2.505-5.954 5.958 0 3.433 2.673 5.96 5.954 5.96 3.282 0 5.955-2.527 5.955-5.96 0-3.453-2.673-5.96-5.955-5.96zm0
                    9.567c-1.8 0-3.35-1.487-3.35-3.61 0-2.14 1.55-3.608 3.35-3.608s3.35 1.46 3.35 3.6c0 2.12-1.55 3.61-3.35 3.61z"></path></g><g id="idEJde">
                    <path fill="#4285f4" d="M14.11 14.182c.722-.723 1.205-1.78 1.387-3.334H9.423V8.373h8.518c.09.452.16 1.07.16 1.664 0 1.903-.52 4.26-2.19
                    5.934-1.63 1.7-3.71 2.61-6.48 2.61-5.12 0-9.42-4.17-9.42-9.29C0 4.17 4.31 0 9.43 0c2.83 0 4.843 1.108 6.362 2.56L14 
                    4.347c-1.087-1.02-2.56-1.81-4.577-1.81-3.74 0-6.662 3.01-6.662 6.75s2.93 6.75 6.67 6.75c2.43 0 3.81-.972 4.69-1.856z"></path></g></svg>
         
          <p className='font-[Google Sans] text-[24px] text-[#202124]'>Create your Google Account</p>
          
          <div className='flex flex-row gap-3 w-[100%] mt-3'>
            <TextField className='w-[50%]' id='first-name' label="First name" variant="outlined" size='small'/>
            <TextField className='w-[50%]' id='last-name' label="Last name" variant="outlined" size='small'/>          
          </div>

          <div className='flex flex-col gap-1 mt-2'>
            <OutlinedInput
              size='small'
              id="username-gmail"
              endAdornment={<InputAdornment position="end">@gmail.com</InputAdornment>}
            />
            <p className='text-[12px] text-[#5f6368] font-[Roboto] pl-2'>You can use letters, numbers, & periods</p>
          </div>

          <div className='flex items-center justify-center h-[35px] w-[250px] hover:bg-blue-50 rounded-[4px]'>
            <p className=' hover:text-[#174ea6] text-blue-600 text-[14px] font-[Google Sans] font-[500]'>Use my current email address instead</p>
          </div>

          <div className='flex flex-col gap-1 mt-4 mb-[-10px]'>
            <div className='flex flex-row gap-3 w-[100%]'>
              <TextField className='w-[50%]' id='password' label="Password" type={show} variant="outlined" size='small'/>
              <TextField className='w-[50%]' id='confirm' label="Confirm" type={show} variant="outlined" size='small'/>          
            </div>
            <p className='text-[12px] text-[#5f6368] font-[Roboto] pl-2'>Use 8 or more characters with a mix of letters, numbers & symbols</p>
          </div>

          {/* <div className='w-[100%] flex flex-row items-center gap-3'>
            <input type='checkbox' />
            <p>Show password</p>
          </div> */}

          <FormControlLabel control={<Checkbox onClick={toggle} />} label="Show password" />

          <div className='flex flex-row justify-between items-center w-[100%] h-[60px]'>
            <Button 
              className='' 
              variant="text">
                <Link  
                  className='hover:text-[#174ea6] text-blue-600 font-[Roboto] text-[14px] capitalize font-[400]' 
                  to="/account/sign-in"
                  >Sign in instead
                </Link>
            </Button>
            <Button 
              className='w-[80px] h-[36px]' 
              variant="contained">
                <Link 
                  className='text-[16px] capitalize font-[Google sans]' 
                  to="https://www.youtube.com/watch?v=dQw4w9WgXcQ"
                  >Next
                  </Link>
            </Button>
          </div>

        </div>
        <div className='flex flex-col w-[45%] h-[100%] items-center pt-16'>
          <img src="https://ssl.gstatic.com/accounts/signup/glif/account.svg" alt="" width="244" height="244" class="j9NuTc TrZEUc" />
          <p className='px-5 text-center font-[Roboto] font-[300] text-[#202124] text-[16px]'>One account. All of Google working for you.</p>
        </div>
      </div>
      <div className='w-[710px] h-[50px] flex flex-row'>
          <div className='w-[50%] h-[100%]'>
          <p className='text-[#3c4043] text-[12px] font-[Roboto]'>English</p>
          </div>
          <div className='flex flex-row justify-end gap-6 w-[50%] h-[100%]'>
            <p className='text-[#3c4043] text-[12px] font-[Roboto]'>Help</p>
            <p className='text-[#3c4043] text-[12px] font-[Roboto]'>Privacy</p>
            <p className='text-[#3c4043] text-[12px] font-[Roboto]'>Terms</p>
          </div>
      </div>
      
    </div>
  )
}
